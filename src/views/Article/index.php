            ...
            foreach ($articles as $article) {
                ...
                <div class="blockCard">
                    <div class="card">
                        <div class="top">
                            ...
                            <form action="/dashboard/<?php echo escape($article->getId_article()); ?>/delete" method="post">
                                ...
                                if (isset($_SESSION["user"])) {
                                    if ($_SESSION["user"]["pseudo"] == escape($article->getPseudoUser()) || $_SESSION["user"]["role"] == 1) {?>
                            ...
                            <form action="/dashboard/<?php echo escape($article->getId_article()); ?>/modification" method="get">
                            <?php
                        ...
                        <form action="/dashboard/<?php echo escape($article->getId_article()); ?>/commentaire" method="post" enctype='multipart/form-data'>
                            <input type="hidden" value="<?php echo escape($article->getId_article()); ?>" name="IdArticleCommente">
                            <p>Photo: <input type="file" name="photo"></p>
                            <textarea name="texte" rows="2" cols="40" placeholder="Commentaire de l'article"><?php echo old("texte");?></textarea>
                            <br>
                            <p align="right"><button >Poster le commentaire</button></p>
                        </form>
                    </div>
                </div>
                                    ...
                                    <form action="/dashboard/<?php echo escape($commentaire->getId_article()); ?>/delete"
                                        ...
                                        if (isset($_SESSION["user"])) {
                                            if ($_SESSION["user"]["pseudo"] == escape($commentaire->getPseudoUser()) || $_SESSION["user"]["role"] == 1) {
                                                ...
                                                ?>
                                                <button class="button btn-danger" type="submit" onclick="return confirm('Etes-vous sur de vouloir supprimer ce métier ?')">
