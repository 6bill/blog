...
class ArticleManager
{
    ...
    public function storeCommentaire()
    {
        ...
        $stmt = $this->bdd->prepare("INSERT INTO article(Titre, Date, Texte, Photo, Id_user,IdArticleCommente ) VALUES (?, NOW(), ?, ?, ?,?)");
        $retour = $stmt->execute(array(
            $titre,
            $_POST["texte"],
            $_FILES['photo']['name'],
    ...
    public function update($id){
        $stmt = $this->bdd->prepare("UPDATE article set (Titre, Date, Texte, Photo, Id_user, IdArticleCommente ) VALUES (?, NOW(), ?, ?, ?)");
        $retour = $stmt->execute(array(
            $_POST["titre"],
            $_POST["texte"],
            $_FILES['photo']['name'],
